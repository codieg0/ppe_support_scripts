...
# re-wrapping silently eats it, which is why a short DMARC record stayed
...
wrap_text() {
    ...
    local inner_width=$(( width - ${#indent} ))
    (( inner_width < 1 )) && inner_width=$width
...
# right registry's RDAP server for the TLD. In practice rdap.org silently
# returns an empty body to curl (confirmed: a real browser hitting that same
# URL gets redirected and gets data back fine, but curl gets nothing back at
# all — almost certainly bot/WAF filtering on rdap.org's side, not something
# fixable with curl flags). That's why domain age always showed "Unknown"
# no matter what domain was searched.
#
# Fix: resolve the correct registry RDAP server ourselves from IANA's own
# bootstrap registry (the same data rdap.org uses internally) and query it
# directly, skipping rdap.org entirely.
rdap_bootstrap_cache="${HOME}/.cache/proofpoint-stacks/rdap_bootstrap.json"
 
# Refreshes the cached IANA RDAP bootstrap file (data.iana.org/rdap/dns.json)
# if it's missing or older than a day. This file rarely changes, so caching
# avoids a network round trip on every single domain lookup.
ensure_rdap_bootstrap() {
    mkdir -p "$(dirname "$rdap_bootstrap_cache")" 2>/dev/null
    if [[ ! -s "$rdap_bootstrap_cache" ]] || \
       [[ -n $(find "$rdap_bootstrap_cache" -mmin +1440 -print -quit 2>/dev/null) ]]; then
        curl -4 -s --connect-timeout 5 --max-time 10 \
            "https://data.iana.org/rdap/dns.json" -o "${rdap_bootstrap_cache}.tmp" 2>/dev/null \
            && [[ -s "${rdap_bootstrap_cache}.tmp" ]] \
            && mv "${rdap_bootstrap_cache}.tmp" "$rdap_bootstrap_cache"
    fi
}
 
# Looks up the RDAP base URL for a given TLD in the cached bootstrap file.
# Returns nothing if the TLD isn't listed (some ccTLDs don't have RDAP yet)
# or the bootstrap file couldn't be fetched.
rdap_base_for_tld() {
    local tld=$1 base=""
    [[ -s "$rdap_bootstrap_cache" ]] || return
 
    if command -v jq >/dev/null 2>&1; then
        base=$(jq -r --arg t "$tld" \
            '.services[] | select(.[0][] == $t) | .[1][0]' \
            "$rdap_bootstrap_cache" 2>/dev/null | head -n1)
    elif command -v python3 >/dev/null 2>&1; then
        base=$(python3 - "$tld" "$rdap_bootstrap_cache" 2>/dev/null <<'EOF'
import json, sys
...
age_lookup() {
    ...
    # -4 avoids the slow AAAA-then-A fallback some networks hit when IPv6
    # routing is broken. A browser-like UA/Accept header is a cheap safety
    # net in case a given registry's RDAP server does its own light bot
    # filtering (most don't, but it costs nothing to include).
    resp=$(curl -4 -sL --connect-timeout 5 --max-time 8 \
...
domain_info() {
    ...
    # separately-registered domain the customer owns — RDAP has no
    # registration record for those, so a re-run against one always comes
    # back empty. Skip the known cases, and — more importantly — never let
    # an empty re-run clobber the good result the first lookup (against the
    # domain you actually typed) already found.
    local age_domain new_age_file
    age_domain=$(head -n1 "$domainsfile" 2>/dev/null)
    if [[ -n "$age_domain" && "${age_domain,,}" != "${domain,,}" \
          && "${age_domain,,}" != *.onmicrosoft.com \
          && "${age_domain,,}" != *.ppe-hosted.com ]]; then
        new_age_file=$(mktemp)
        age_lookup "$age_domain" > "$new_age_file" 2>/dev/null
        if [[ -s "$new_age_file" ]]; then
            mv "$new_age_file" "$agefile"
        else
            rm -f "$new_age_file"
            age_domain=$domain
        fi
